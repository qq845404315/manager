<template>
	<div class="app-container">
		...
		<!--工具条-->
		<el-pagination layout="total, prev, pager, next" background :page-size="10" @size-change="handleSizeChange" :total="total"
			...
			<el-form label-width="80px" :model="temp" ref="dataForm">
				...
				<el-form-item label="状态" v-model="temp.status">
					<el-select v-model="temp.status" placeholder="启用状态">
						<el-option v-for="item in status" :label="item.label" :value="item.statusId" :key="item.statusId"></el-option>
					</el-select>
				</el-form-item>
			</el-form>
		...
		methods: {
			fetchData() {
				...
				getList(this.listQuery).then(response => {
					const limit = 10
					const pageList = response.data.filter((item, index) => index < limit * this.page && index >= limit * (this.page -
						1))
